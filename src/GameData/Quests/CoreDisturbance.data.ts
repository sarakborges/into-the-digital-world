import { ALL_CORES } from '@/Consts/Cores.const'

import type { QuestType } from '@/Types/Quest.type'

export const CORE_DISTURBANCE: QuestType = {
  id: `CORE_DISTURBANCE`,
  name: `Core disturbance`,
  description: `Tsumemon are causing distress around Root Domain! That means we can't travel safely anywhere.\n\nI need you to go to Root Core, find who's boss, and give them a lesson. That way, everyone - you included -, can travel once more. Come back once you done so.`,
  completionText: `Thank you, traveler! We hope that now everyone can travel normally.`,
  ongoingText: `I see you still haven't dealt with the problem. Not trying to be too pushy, but can you hurry? We depend on you.`,

  objectives: [
    {
      id: 'ROOT_CORE_TSUMEMON_ELITE_KILL',
      type: 'SPECIFIC_ENEMY',
      text: `Elite Tsumemon defeated at Root Core`,
      enemyId: 'ROOT_CORE_TSUMEMON_ELITE',
      quantity: 1
    }
  ],

  rewards: {
    currency: 1,
    exp: 1,
    updatedRegion: `ROOT_FORK`,
    cores: {
      [ALL_CORES.virus.id]: 2,
      [ALL_CORES.nightmareSoldiers.id]: 2
    }
  }
}
